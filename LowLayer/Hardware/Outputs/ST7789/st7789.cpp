...
static qcPeripheral Peripheral;

#define LCD_SPI_OBJECT Peripheral.LCD_SPI_HANDLE
#define LCD_TIM Peripheral.htim2
...
static void ST7789_WriteCommand(uint8_t cmd)
{
	ST7789_Select();
	ST7789_DC_Clr();
	HAL_SPI_Transmit(&LCD_SPI_OBJECT, &cmd, sizeof(cmd), 1000);
	ST7789_UnSelect();
}
...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
	...
	while (buff_size > 0) {
		uint16_t chunk_size = buff_size > 65535 ? 65535 : buff_size;
			...
			HAL_SPI_Transmit(&LCD_SPI_OBJECT, buff, chunk_size, 1000);
...
static void ST7789_WriteSmallData(uint8_t data)
{
	ST7789_Select();
	ST7789_DC_Set();
	HAL_SPI_Transmit(&LCD_SPI_OBJECT, &data, sizeof(data), 1000);
	ST7789_UnSelect();
}
...
//			ST7789_WriteData((ui8*)disp_buf, sizeof(disp_buf));
//		}
//	#else
		uint16_t j;
...
{
	ST7789_Select();
	ST7789_SetAddressWindow(x, y, x + font->chars[(ch-32)].image->width - 1, y + font->chars[(ch-32)].image->height - 1);
	...
	for (int i = 0; i < font->chars[(ch-32)].image->height; i++)
		for (int k = 0; k < font->chars[(ch-32)].image->width; k++)
		{
			...
			if (alpha>0){
				pixel=Paint(color,bgcolor,alpha);
				ST7789_DrawPixel(k, i, pixel);
			}
			else
				ST7789_DrawPixel(k, i, bgcolor);
...
void ST7789_WriteString_Test(uint16_t x, uint16_t y, const char *str, tFont *font, uint16_t color, uint16_t bgcolor)
{
	...
	while (*str) {
		...
		ST7789_WriteChar_Test(x, y, *str, font, color, bgcolor);
...
void init_board (void)
{
	...
	LCD_TIM.Instance = TIM2;

	LCD_TIM.Init.Prescaler         	= 110;
	LCD_TIM.Init.Period            	= 1000;
	LCD_TIM.Init.ClockDivision     	= 0;
	LCD_TIM.Init.CounterMode       	= TIM_COUNTERMODE_UP;
	LCD_TIM.Init.RepetitionCounter 	= 0;
	LCD_TIM.Init.AutoReloadPreload 	= TIM_AUTORELOAD_PRELOAD_DISABLE;

	if (HAL_TIM_PWM_Init(&LCD_TIM) != HAL_OK)
	{
	...
	if (HAL_TIM_PWM_ConfigChannel(&LCD_TIM, &sConfigOC, TIM_CHANNEL_1) != HAL_OK)
		/* Configuration Error */
		HardwareErrorHandler();

	/* Start PWM channel */
	if (HAL_TIM_PWM_Start(&LCD_TIM, TIM_CHANNEL_1) != HAL_OK)
	...
	LCD_SPI_OBJECT.Instance = LCD_SPI_INSTANCE;

	LCD_SPI_OBJECT.Init.Mode					= SPI_MODE_MASTER;
	LCD_SPI_OBJECT.Init.Direction			= SPI_DIRECTION_1LINE;
	LCD_SPI_OBJECT.Init.DataSize				= SPI_DATASIZE_8BIT;
	LCD_SPI_OBJECT.Init.CLKPolarity			= SPI_POLARITY_HIGH;
	LCD_SPI_OBJECT.Init.CLKPhase				= SPI_PHASE_2EDGE;
	LCD_SPI_OBJECT.Init.NSS					= SPI_NSS_SOFT;
	LCD_SPI_OBJECT.Init.BaudRatePrescaler	= SPI_BAUDRATEPRESCALER_2;
	LCD_SPI_OBJECT.Init.FirstBit				= SPI_FIRSTBIT_MSB;
	LCD_SPI_OBJECT.Init.TIMode				= SPI_TIMODE_DISABLE;
	LCD_SPI_OBJECT.Init.CRCCalculation		= SPI_CRCCALCULATION_DISABLE;

	if (HAL_SPI_Init(&LCD_SPI_OBJECT) != HAL_OK)
		HardwareErrorHandler();

	SPI_1LINE_TX(&LCD_SPI_OBJECT);
	__HAL_SPI_ENABLE(&LCD_SPI_OBJECT);
